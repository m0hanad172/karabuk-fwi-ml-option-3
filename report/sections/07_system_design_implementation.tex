...
YOLOv8 \texttt{best.pt} detector to identify fire and smoke evidence from
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{system_architecture_firewatch.png}
  \caption{FireWatch system architecture connecting the React/TypeScript dashboard, FastAPI backend, prediction services, YOLOv8 monitoring services, scheduler, drone-ready service, and SQLite persistence.}
  \label{fig:system-architecture}
\end{figure}

Figure~\ref{fig:system-architecture} presents the implemented architecture.
The design keeps prediction services and monitoring services separate while
allowing the dashboard to read both through the backend API.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.94\textwidth]{sqlite_erd_firewatch.png}
  \caption{SQLite persistence design showing the main runtime tables used by FireWatch: prediction history, system state, and detection alerts.}
  \label{fig:sqlite-erd}
\end{figure}

Figure~\ref{fig:sqlite-erd} shows the key runtime entities. The same SQLite
database file is used for local persistence, but the monitoring layer writes
only detection alert records and does not create prediction-history rows.
...
\texttt{best.pt} detector and records visible fire or smoke evidence. Keeping
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{alert_workflow_firewatch.png}
  \caption{Detection alert workflow from a YOLOv8 detection event to persistent unread alerts, operator review, read/unread state, and soft deletion.}
  \label{fig:alert-workflow}
\end{figure}

Figure~\ref{fig:alert-workflow} explains the alert lifecycle implemented in
the dashboard. The workflow supports repeated operator review instead of
treating detections as temporary notifications.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{runtime_sequence_firewatch.png}
  \caption{Runtime sequence for scheduled risk checks, feature generation, two-stage inference, SQLite storage, and dashboard update.}
  \label{fig:runtime-sequence}
\end{figure}

Figure~\ref{fig:runtime-sequence} shows that scheduled runs and manual runs
share the same inference and persistence path. This consistency makes
operational decisions easier to audit.

\section{Drone-Ready Implementation}

FireWatch includes a drone-ready layer for supervised aerial monitoring with
operator-controlled DJI Tello testing. This layer connects camera-based
monitoring with a real drone video source while keeping physical drone
operation under explicit operator control. It supports the project goal of
being drone-ready and hardware-testable without claiming certified autonomous
deployment.

The current implementation supports two execution paths: mock mode and Tello
mode. Mock mode remains available for development, demonstrations without
hardware, interface checks, and automated testing. Tello mode allows the system
to work with a configured DJI Tello drone, including connection status, video
stream handling, battery/status reporting, emergency stop support, and a short
controlled demo patrol when the required safety flags are enabled.
...
detection path can process frames from the drone-ready video source, allowing
the Tello stream to reuse the alerting and persistence design already used for
local camera feeds.
...
configuration, sufficient battery status, an active Tello connection, and
operator confirmation. Emergency stop support is available through the
drone-ready API layer. This design keeps the prototype safe for academic
demonstration and prevents risk prediction from directly causing uncontrolled
hardware movement.
